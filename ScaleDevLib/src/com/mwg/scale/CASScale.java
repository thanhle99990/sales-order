...
public class CASScale implements IScale {
  ...
  public long getValueInGam() throws Exception {
    ...
    final Map<String, String> scaleInfo = CASHelper.makeScaleInfo(this.ip, CASScale.port, EmScaleModel.PRII_D_VN);
    cas.sendToScale(scaleEnv, null, scaleInfo, new ResultReceive() {
      ...
      public void setResultData(Map<String, String> scaleEnv, int workResult, List<Map<String, String>> takeData, Object objSendList, Map<String, String> scaleInfo) {
        try {
          ...
          switch (emState) {
          case SUCCESS:
            result.setValue(CASScale.getWeightFromData(takeData));
            break;
          case FAIL:
            result.setValue(CASScale.getWeightFromData(takeData));
            break;
    ...
    if (resultIter.hasNext()) {
      ...
      if (errCode != null) {
        ...
        case WeightVo.STATE_ERR_STABLE:
          return Long.parseLong(resultMap.get(MapKeyWeightInfo.Weight));
